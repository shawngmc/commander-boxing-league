// Custom version that uses the Google Apps Script cache API
...
const scryfallSearch_ = (params, num_results = MAX_RESULTS_) => {
  ...
  try {
    while (true) {
      response = JSON.parse(cachedUrlFetchContentText_(`${scryfall_url}&page=${page}`));
    ...
    throw new Error(`Unable to retrieve results from Scryfall: ${error} on url ${scryfall_url}`);
  }
